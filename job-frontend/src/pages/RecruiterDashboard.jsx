...
export default function RecruiterDashboard() {
  ...
  if (user.role !== "recruiter") {
    return (
      <div style={styles.errorContainer}>
        <p>Access denied. Recruiters only. Your role: {user.role}</p>
      </div>
    );
  }
  ...
  const menuItems = [
    { path: "/recruiter/post-job", label: "Post Job", icon: PostJobIcon },
    { path: "/recruiter/my-jobs", label: "My Jobs", icon: MyJobsIcon },
  ];
        ...
        <div style={styles.userInfo}>
          ...
          {sidebarOpen && (
            <div style={styles.userDetails}>
              <p style={styles.userName}>{user.name || 'Recruiter'}</p>
        ...
        <header style={styles.navbar}>
          <div style={styles.navbarLeft}>
            <h2 style={styles.pageTitle}>
              {menuItems.find(item => isActive(item.path))?.label || 'Recruiter Dashboard'}
            </h2>
          </div>
